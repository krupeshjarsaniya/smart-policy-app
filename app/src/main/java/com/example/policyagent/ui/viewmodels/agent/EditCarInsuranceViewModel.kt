...
class EditCarInsuranceViewModel (
    ...
    fun editCarInsurance(addCarInsurance: EditCarInsurance, id: String, mContext: Context){
        listener?.onStarted()
        Coroutines.main {
            try {
                var gson = Gson()
                ...
                map["documentsRemoveDataArray"] = addCarInsurance.documentsRemoveDataArray!!.toRequestBody("multipart/form-data".toMediaTypeOrNull())
                ...
                val response = Gson().fromJson(repository.editCarInsurance(map,id), CommonResponse::class.java)
                if (response.status!!){

...
class EditPolicyViewModel (
    ...
    fun editPolicy(addPolicy: AddPolicy,id: String, mContext: Context) {
        listener?.onStarted()
        Coroutines.main {
            try {
                var gson = Gson()
                ...
                val response = Gson().fromJson(repository.editPolicy(map,id), CommonResponse::class.java)
                if (response.status!!){

...
class EditHealthInsuranceViewModel (
    ...
    fun editHealthInsurance(addHealthInsurance: EditHealthInsurance, id: String, mContext: Context){
        listener?.onStarted()
        Coroutines.main {
            try {
                var gson = Gson()
                ...
                val response = Gson().fromJson(repository.editHealthInsurance(map,id), CommonResponse::class.java)
                if (response.status!!){

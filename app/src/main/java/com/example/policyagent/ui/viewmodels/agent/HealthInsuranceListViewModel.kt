...
class HealthInsuranceListViewModel (
    ...
    fun getHealthInsurance(mContext: Context, page: Int){
        listener?.onStarted()
        Coroutines.main {
            try {
                val map = HashMap<String, Any>()
                map["page_no"] = page.toString()
                val response = Gson().fromJson(repository.getHealthInsurance(map), HealthInsuranceListResponse::class.java)
                if (response.status!!){
    ...
    fun deleteHealthInsurance(mContext: Context, id: String, position: Int){
        listener?.onStarted()
        Coroutines.main {
            try {
                val response = Gson().fromJson(repository.deleteHealthInsurance(id), CommonResponse::class.java)
                if (response.status!!){
                    response.let {

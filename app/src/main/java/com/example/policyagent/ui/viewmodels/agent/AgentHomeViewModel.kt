...
class AgentHomeViewModel (
    ...
    var listener: AgentDashBoardListener? = null
    ...
    fun getAgentDashboard(mContext: Context){
        listener?.onStarted()
        Coroutines.main {
            try {
                val response = Gson().fromJson(repository.getAgentDashboard(), AgentDashBoardResponse::class.java)
                if (response.status!!){

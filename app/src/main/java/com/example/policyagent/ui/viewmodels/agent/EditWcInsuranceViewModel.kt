...
class EditWcInsuranceViewModel (
    ...
    fun editWcInsurance(addWcInsurance: EditWcInsurance, id: String, mContext: Context){
        listener?.onStarted()
        Coroutines.main {
            try {
                var gson = Gson()
                ...
                map["premium_amount"] = addWcInsurance.net_preminum!!.toRequestBody("multipart/form-data".toMediaTypeOrNull())
                ...
                map["documentsRemoveDataArray"] = addWcInsurance.documentsRemoveDataArray!!.toRequestBody("multipart/form-data".toMediaTypeOrNull())
                ...
                val response = Gson().fromJson(repository.editWcInsurance(map,id), CommonResponse::class.java)
                if (response.status!!){
